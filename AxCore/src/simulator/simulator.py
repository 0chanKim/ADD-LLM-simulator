...
from AxCore.src.utils.utils import ceil_a_by_b, log2, lookup_pandas_dataframe
from AxCore.src.simulator.stats import Stats
from AxCore.src.simulator.loop_stack import LoopStack
from AxCore.src.optimizer.optimizer import optimize_for_order, get_stats_fast
from AxCore.src.simulator.accelerator import Accelerator
from AxCore.src.simulator.energy import EnergyTuple

from AxCore.sram.cacti_sweep import CactiSweep
import os
import pandas

from dnnweaver2.tensorOps.cnn import Convolution, MatMul

class AxCoreSimulator(object):
    """
    Simulator class
    """

    def __init__(self, config_file='conf.ini', synth_csv=None, verbose=False, energy_costs=None):
        ...
        self.module_name = self.config.get('module', 'module')
    ...
    def get_energy_cost(self):
        ...
        tech_node = 0.028
        ##################################################
        cfg_dict = {'size (bytes)': wbuf_bank_size /8., 'block size (bytes)': wbuf_bits/8., 'technology (u)': tech_node, 'read-write port': 0}
        ...
        self.logger.debug('\tWrite Energy                : {0:>8.4f} pJ/bit'.format(wbuf_write_energy * 1.e3))
        ##################################################
        cfg_dict = {'size (bytes)': ibuf_bank_size /8., 'block size (bytes)': ibuf_bits/8., 'technology (u)': tech_node, 'read-write port': 0}
        ibuf_data = self.sram_obj.get_data_clean(cfg_dict)
        # print(ibuf_data)
        ibuf_read_energy = float(ibuf_data['read_energy_nJ'].iloc[0]) / ibuf_bits
        ibuf_write_energy = float(ibuf_data['write_energy_nJ'].iloc[0]) / ibuf_bits
        ...
        self.logger.debug('\tWrite Energy                : {0:>8.4f} pJ/bit'.format(ibuf_write_energy * 1.e3))
        ##################################################
        cfg_dict = {'size (bytes)': obuf_bank_size /8., 'block size (bytes)': obuf_bits/8., 'technology (u)': tech_node, 'read-write port': 1}
        obuf_data = self.sram_obj.get_data_clean(cfg_dict)
        obuf_read_energy = float(obuf_data['read_energy_nJ'].iloc[0]) / obuf_bits
        obuf_write_energy = float(obuf_data['write_energy_nJ'].iloc[0]) / obuf_bits
        obuf_leak_power = float(obuf_data['leak_power_mW'].iloc[0]) * obuf_bank
        ...
        core_data = lookup_pandas_dataframe(core_synth_data, lookup_dict)
        if len(core_data) == 0:
            lookup_dict['Module'] = self.module_name
            ...
            core_leak_power = float(core_data['Leakage Power (nW)'].iloc[0]) * (N * M) / 1.
            core_leak_energy = core_leak_power / float(core_data['Frequency'].iloc[0])
            core_weight_bits = float(core_data['Weight (bits)'].iloc[0])
            core_act_bits = float(core_data['Activation (bits)'].iloc[0])
        else:
            core_area = float(core_data['Area (um^2)'].iloc[0]) * 1.e-6
            core_dyn_power = float(core_data['Dynamic Power (nW)'].iloc[0])
            core_dyn_energy = core_dyn_power / float(core_data['Frequency'].iloc[0])
        ...
        self.logger.debug('Core :')
        self.logger.debug('\tDimensions              : {0}x{1}-systolic array'.format(N, M))
        self.logger.debug('\tWeight bits             : {}'.format(core_weight_bits))
        self.logger.debug('\tActivation bits         : {}'.format(core_act_bits))
        self.logger.debug('\tLeak power              : {} (nW)'.format(core_leak_energy))
        self.logger.debug('\tDynamic Energy (nJ)     : {}'.format(core_dyn_energy))
        self.logger.debug('\tArea (mm^2)             : {}'.format(core_area))
        ##################################################
    ...
    def get_conv_cycles(self, K, O, S, IC, OC, iprec, wprec, batch_size=1, im2col=True, weight_stationary = False):
        ...
        lookup_dict['Module'] = self.module_name
        core_data = lookup_pandas_dataframe(core_synth_data, lookup_dict)
        weight_bits = int(core_data['Weight (bits)'].iloc[0])
        act_bits = int(core_data['Activation (bits)'].iloc[0])
        iprec = act_bits
        wprec = weight_bits
        ...
        # print(f"best_instructions: {best_instructions}")
        # print(f"best_tiling: {best_tiling}")
    ...
    def get_cycles(self, op, im2col=True, weight_stationary = False):
        if isinstance(op, Convolution):
            ...
            # print(f"op.data.shape: {op.data.shape}")
            B = op.data.shape[0]
