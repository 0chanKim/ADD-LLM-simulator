...
import AxCore.src.benchmarks.ant_bench as ant
import AxCore.src.benchmarks.adafloat_bench as ada
import AxCore.src.benchmarks.bitfusion_bench as bit
import AxCore.src.benchmarks.olaccel_bench as ola
import AxCore.src.benchmarks.ant_weight_bench as ant_weight
import AxCore.src.benchmarks.biscaled_bench as biscaled
import AxCore.src.benchmarks.axcore_bench as axcore
...
def create_net(net_name, net_list, batch_size):
    ...
    with g.as_default():
        for idx, op in enumerate(net_list):
            ...
            # print(input_size, kernel_size, output_size, kernel_stride, padding, precision, op_type)
            ...
            if op_type == 0:
                with g.name_scope('conv'+str(idx)):
                    out = create_conv(input_size, kernel_size, stride_size=kernel_stride, pad=padding, c_dtype=FQDtype.FP16, w_dtype=precision)
                ...
                with g.name_scope('fc'+str(idx)):
                    out = create_fc(input_size, kernel_size, c_dtype=FQDtype.FP16, w_dtype=precision)
                    # print(idx, op, out.shape)
                    assert out.shape[0] == output_size[0]
...
def create_fc(input_size, weight_size, c_dtype=None, w_dtype=None):
    ...
    input = get_tensor(shape=(batch_size, input_size[1]), name='data', dtype=FQDtype.FP16, trainable=False)
...
def get_bench_nn_axcore(bench_name, batch_size):
    if bench_name == 'opt_125m':
        net_list = copy.deepcopy(axcore.opt_125m)
        return create_net(bench_name, net_list, batch_size)
    elif bench_name == 'opt_350m':
        net_list = copy.deepcopy(axcore.opt_350m)
        return create_net(bench_name, net_list, batch_size)
    elif bench_name == 'opt_1_3b':
        net_list = copy.deepcopy(axcore.opt_1_3b)
        return create_net(bench_name, net_list, batch_size)
    elif bench_name == 'opt_2_7b':
        net_list = copy.deepcopy(axcore.opt_2_7b)
        return create_net(bench_name, net_list, batch_size)
    elif bench_name == 'opt_6_7b':
        net_list = copy.deepcopy(axcore.opt_6_7b)
        return create_net(bench_name, net_list, batch_size)
    elif bench_name == 'opt_13b':
        net_list = copy.deepcopy(axcore.opt_13b)
        return create_net(bench_name, net_list, batch_size)
    elif bench_name == 'opt_30b':
        net_list = copy.deepcopy(axcore.opt_30b)
        return create_net(bench_name, net_list, batch_size)
...
def get_bench_numbers(graph, sim_obj, batch_size=1, weight_stationary = False):
    ...
    for opname, op in graph.op_registry.items():
        # print(f"opname: {opname}, op.data.shape: {op.data.shape}")
